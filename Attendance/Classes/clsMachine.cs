        ...
        private bool GetMachineInfoFromDb()
        {
            ...
            if (hasrows)
            {
                foreach (DataRow dr in ds.Tables[0].Rows)
                {
                    ...
                    _firstinLastout = Convert.ToBoolean(dr["FirstInLastOut"]);

                    if (_lunchinout)
                        _tableName = "AttdLunchGate";
                    else if (_firstinLastout)
                        _tableName = "AttdIO";
                    else if (_messflg)
                        _tableName = "AttdLog";
                    else if (_gateinout)
                    ...
                    using (SqlConnection cn = new SqlConnection(Utils.Helper.constr))
                    {
                        try
                        {
                            ...
                            using (SqlCommand cmd = new SqlCommand())
                            {
                                ...
                                cmd.CommandText = "sp_FirstIn_LastOut_Process";
                                ...
                                cmd.Parameters.AddWithValue("@pEmpUnqID", t.EmpUnqID);
                                cmd.Parameters.AddWithValue("@punchDt", t.PunchDate.ToString("yyyy-MM-dd HH:mm:ss"));
            ...
            if(this.CZKEM1.SetStrCardNumber(emp.CardNumber))
            {
                if (this.CZKEM1.SSR_SetUserInfo(_machineno, emp.UserID, "", "", 0, emp.Enabled))
                {
                    ...
                    if (_messflg == false)
                    {
                        ...
                        this.CZKEM1.SetUserInfoEx(_machineno, Convert.ToInt32(emp.UserID), 0, 0);
                    }
                }
