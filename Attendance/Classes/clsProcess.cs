...
namespace Attendance
{
    ...
    public class clsProcess
    {
                        ...
                        foreach (DataRow drDate in dsDate.Tables[0].Rows)
                        {
                            ...
                            if (hasRows)
                            {
                                ...
                                foreach (DataRow drAttd in dsAttdData.Tables[0].Rows)
                                {
                                    ...
                                    if (tconswrkhrs >= 4 && tconswrkhrs < 8 && drAttd["Status"].ToString() == "P")
                                    {
        ...
        public void ArrivalProcess(DateTime tFromDt, DateTime tToDate, string CoverShift, out int result)
        {
            result = 0;
            ...
            using (SqlConnection cn = new SqlConnection(Utils.Helper.constr))
            {
                try
                {
                    ...
                    using (SqlCommand cmd = new SqlCommand())
                    {
                        ...
                        cmd.Parameters.AddWithValue("@pFromDt", tFromDt);
                        cmd.Parameters.AddWithValue("@pToDt", tToDate);
                        cmd.Parameters.AddWithValue("@CoverShift", CoverShift);
        ...
        public void CalcShiftOT(SqlDataAdapter daAttdData, DataSet dsAttdData, DataRow drAttd, clsEmp Emp, string tSchShift, out string err)
        {
            ...
            try
            {
                ...
                Boolean tMaxOTExp = false;
                double tMaxOt = 0;
                //check if Emp has LateCome exception
                tcnt = Convert.ToInt32(Utils.Helper.GetDescription("Select Count(*) From MastException Where EmpUnqID ='" + drAttd["EmpUnqID"].ToString() + "' and ExecMaxOT = 1", Utils.Helper.constr));
                if (tcnt > 0)
                {
                    tMaxOTExp = true;
                    tMaxOt = Convert.ToDouble(Utils.Helper.GetDescription("Select MaxOT From MastException Where EmpUnqID ='" + drAttd["EmpUnqID"].ToString() + "' and ExecMaxOT = 1", Utils.Helper.constr));
                }
                ...
                if (eWrkGrp == "CONT")
                {
                    if (tShift == "WO" || tShift == "HL" || tShift == "OH" )
                    {
                        tShift = "";
                    }
                    else
                ...
                if (eWrkGrp == "COMP")
                {
                    if (string.IsNullOrEmpty(Emp.GradeCode))
                        eGradeCode = 99999;
                    else
                ...
                else
                {
                    ...
                    if (drShiftC.GetLength(0) == 0 && eWrkGrp != "CONT" )
                    {
                    ...
                    else if (drShiftC.GetLength(0) == 0 && eWrkGrp == "CONT" && tShift != "OP")                    
                    {
                        //auto shift calc
                        if (tInTime != DateTime.MinValue && tOutTime.HasValue)
                        {
                    ...
                    {
                        //22-11-2019 : Unknown shift->make it open and calculate, (Contractual Employee)
                    ...
                    if (tInTime >= ShiftInFrom)
                    {
                        ...
                        if (tInTime < ShiftStart)
                        {
                            ...
                            //if employee did not come as per scheduled shift it shows absent
                        ...
                        if (drAttd["ConsOut"] != DBNull.Value)
                        {
                            ...
                            if (ShiftEnd > Convert.ToDateTime(drAttd["ConsOut"]))
                            {
                                ...
                                if (t5.TotalSeconds < (-1 * Globals.G_EarlyGoingSec) && t5.TotalSeconds < 0)
                                {
                                    ...
                                    if (Globals.G_HFFLG_EarlyGoing)
                                    {
                                        if (eWrkGrp == "COMP" && eGradeCode <= Globals.G_HFSEC_EarlyGoing)
                                        {
                                            drAttd["Halfday"] = 0;
                                            drAttd["EarlyGoing"] = "";
                                            drAttd["GracePeriod"] = "";
            ...
            OTCalc:
                ...
                if (tOTFLG && (Convert.ToDouble(drAttd["ConsWrkHrs"])) > ShiftHrs && drAttd["LeaveTyp"].ToString() == "")
                {
                    ...
                    if (tHour >= 1)
                    {
                        ...
                        if (ot >= 1)
                        {
                            if (tMaxOTExp && tMaxOt > 0)
                            {
                                if (ot >= tMaxOt)
                                {
                                    drAttd["ConsOverTime"] = tMaxOt;
                                    drAttd["CalcOvertime"] = tMaxOt;
                                }
                ...
                else if (tOTFLG && (drAttd["LeaveTyp"].ToString() == "WO" || drAttd["LeaveTyp"].ToString() == "HL"))
                {
                    ...
                    if (OverTime >= 2)
                    {
                        if (tMaxOTExp && tMaxOt > 0 && OverTime >= 10 )
                        {
                            if (OverTime >= tMaxOt)
                            {
                                //Fix the Overtime 10;
                                drAttd["ConsOverTime"] = 10;
                                drAttd["CalcOvertime"] = 10;
                            }
                        }
                        else
                        {
