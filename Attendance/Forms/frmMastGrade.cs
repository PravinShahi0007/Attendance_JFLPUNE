...
{
    public partial class frmMastGrade : Form
    {
        ...
        private void frmMastGrade_Load(object sender, EventArgs e)
        {
            ResetCtrl();
        ...
        private string DataValidate()
        {
            ...
            if (string.IsNullOrEmpty(txtGradeCode.Text))
            {
                err = err + "Please Enter Grade Code" + Environment.NewLine;
            }
            else
            {
                string input = txtGradeCode.Text.Trim().ToString();
                bool t = Regex.IsMatch(input, @"^\d+$");
                if (!t)
                {
                    err = err + "Please Enter Grade Code in Numeric Format..(001,123,012) " + Environment.NewLine;
                }
                

            }

            if (string.IsNullOrEmpty(txtGradeDesc.Text))
            {
                err = err + "Please Enter Grade Name" + Environment.NewLine;
            }
            ...
            using (SqlConnection cn = new SqlConnection(Utils.Helper.constr))
            {
                using (SqlCommand cmd = new SqlCommand())
                {
                    try
                    {
                        cn.Open();
                        cmd.Connection = cn;
                        string sql = "Insert into MastGrade (CompCode,WrkGrp,GradeCode,GradeDesc,AddDt,AddID) Values ('{0}','{1}','{2}','{3}',GetDate(),'{4}')";
                        sql = string.Format(sql, txtCompCode.Text.Trim().ToString(), 
                            txtWrkGrpCode.Text.Trim().ToString(),
                            txtGradeCode.Text.Trim().ToString(),
                            txtGradeDesc.Text.Trim().ToString(),
                            Utils.User.GUserID);
        ...
        {
            if ( txtGradeCode.Text.Trim() != "" && mode == "NEW" && GRights.Contains("A") )
            {
                btnAdd.Enabled = true;
                btnUpdate.Enabled = false;
                btnDelete.Enabled = false;
            }
            else if (txtGradeCode.Text.Trim() != "" && mode == "OLD")
            {
                btnAdd.Enabled = false;
            ...
            using (SqlConnection cn = new SqlConnection(Utils.Helper.constr))
            {
                using (SqlCommand cmd = new SqlCommand())
                {
                    try
                    {
                        cn.Open();
                        cmd.Connection = cn;
                        string sql = "Update MastGrade Set GradeDesc = '{0}', UpdDt = GetDate(), UpdID = '{1}' Where CompCode = '{2}' " + 
                             " and WrkGrp = '{3}' and GradeCode = '{4}' ";

                        sql = string.Format(sql,  txtGradeDesc.Text.Trim(),
                             Utils.User.GUserID, txtCompCode.Text.Trim().ToString(), txtWrkGrpCode.Text.Trim(), txtGradeCode.Text.Trim()
                           );
        ...
        private void txtGradeCode_KeyDown(object sender, KeyEventArgs e)
        {
            ...
            if (e.KeyCode == Keys.F1 || e.KeyCode == Keys.F2)
            {
                ...
                sql = "Select GradeCode,GradeDesc From MastGrade Where CompCode ='" + txtCompCode.Text.Trim() + "' and WrkGrp = '" + txtWrkGrpCode.Text.Trim() + "' ";
                if (e.KeyCode == Keys.F1)
                {
        ...
        private void txtGradeCode_Validated(object sender, EventArgs e)
        {
            ...
            DataSet ds = new DataSet();
            string sql = "select * From MastGrade where CompCode ='" + txtCompCode.Text.Trim() + "' " +
                    " and WrkGrp='" + txtWrkGrpCode.Text.Trim() + "' and GradeCode ='" + txtGradeCode.Text.Trim() + "'";
