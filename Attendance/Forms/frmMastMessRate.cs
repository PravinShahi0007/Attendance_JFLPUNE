...
{
    public partial class frmMastMessRate: Form
    {
        public string mode = "NEW";
        public string GRights = "XXXV";
        public string oldCode = "";

        public frmMastMessRate()
        {
            InitializeComponent();
        }

        private void frmMastMessRate_Load(object sender, EventArgs e)
        {
            ResetCtrl();
        ...
        private string DataValidate()
        {
            ...
            {
                err = err + "Please Enter MessGrpName" + Environment.NewLine;
            }
            ...
            if (txtFoodRate.EditValue == null)
            {
                err = err + "Please Enter Food Rate.." + Environment.NewLine;
            }
            ...
            using (SqlConnection cn = new SqlConnection(Utils.Helper.constr))
            {
                using (SqlCommand cmd = new SqlCommand())
                {
                    try
                    {
                        cn.Open();
                        cmd.Connection = cn;
                        string sql = "Insert into MastMessRate (CompCode,UnitCode,MessCode,MessGrpCode,FoodCode,FoodRate,AddDt,AddID) Values " +
                        ...
                        sql = string.Format(sql, txtCompCode.Text.Trim().ToString(),
                            ...
                            txtFoodRate.EditValue.ToString(),                            
                            Utils.User.GUserID);
            ...
            using (SqlConnection cn = new SqlConnection(Utils.Helper.constr))
            {
                using (SqlCommand cmd = new SqlCommand())
                {
                    try
                    {
                        cn.Open();
                        cmd.Connection = cn;
                        string sql = "Update MastMessRate Set FoodRate = '{0}', UpdDt = GetDate(), UpdID = '{1}' " +
                            " Where CompCode = '{2}' and UnitCode = '{3}' and MessCode = '{4}' and MessGrpCode = '{5}' and FoodCode = '{6}'";
                        ...
                        sql = string.Format(sql, txtMessDesc.Text.ToString(),
                             ...
                             txtMessGrpCode.Text.Trim(),txtFoodCode.Text.Trim()
                           );
        ...
        private void txtFoodCode_Validated(object sender, EventArgs e)
        {
            ...
            DataSet ds = new DataSet();
            string sql = "select a.*,b.FoodDesc From MastMessRate a,MastMessFood b where a.CompCode ='" + txtCompCode.Text.Trim() + "' " +
            ...
            if (hasRows)
            {
                foreach (DataRow dr in ds.Tables[0].Rows)
                {
                    ...
                    txtFoodRate.EditValue = dr["FoodRate"].ToString(); 
                    txtCompCode_Validated(sender, e);
                    txtUnitCode_Validated(sender, e);
        ...
        private void txtFoodDesc_EditValueChanged(object sender, EventArgs e)
        {
        ...
        private void txtFoodCode_EditValueChanged(object sender, EventArgs e)
        {
